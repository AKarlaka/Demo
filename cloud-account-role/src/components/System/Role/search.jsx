...
const feature = ({
  ...
  const showConfirm = (e) => {
    ...
    let roleNumber = 0;
    let content = '';
    const CheckedRoleNumber = checkedRole.length;
    switch (info) {
      case '删除':
        roleNumber = checkedRole.length;
        content = `删除选中的${roleNumber}个员工！`;
        break;
      case '启用':
        roleNumber = checkedRoleBlockId.length;
        content = `当前选中${CheckedRoleNumber}个员工，可启用${roleNumber}个员工！`;
        break;
      case '停用':
        roleNumber = checkedRoleStartId.length;
        content = `当前选中${CheckedRoleNumber}个员工，可停用${roleNumber}个员工！`;
        break;
      default:
        break;
    }
  ...
  return (<div className="components-search search">
    ...
    <div className="action-box">
      <Row >
        <Col span={16}>
          {roleButton && roleButton.add && <Button type="primary" onClick={onAdd}>+ 新增角色</Button>}
          {roleButton && roleButton.off && <Button onClick={showConfirm} value="停用" disabled={blockBtnStatus} loading={loading}>停用</Button>}
          {roleButton && roleButton.on && <Button onClick={showConfirm} value="启用" disabled={startBtnStatus} loading={loading}>启用</Button>}
          {roleButton && roleButton.delete && <Button onClick={showConfirm} value="删除" disabled={deleteBtnStatus} loading={loading}>删除</Button>}
        </Col>
      </Row>
    </div>
  </div>);
...
feature.propTypes = {
  ...
  checkedRoleStartId: PropTypes.array, // 选中可停用员工id
  checkedRoleBlockId: PropTypes.array, // 选中可启用员工id
