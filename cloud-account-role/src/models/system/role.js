...
import { queryRole, updateStatus, removeRole, createRole, updateRole, queryPostList } from '../../services/system/role';
...
export default {
  ...
  effects: {
    ...
    * queryList({ payload }, { call, put, select }) {
      ...
      const res = yield call(queryRole, parse(payload));
    ...
    * addRole({ payload }, { call, put }) {
      yield put({ type: 'showLoading' });
      const res = yield call(createRole, parse(payload));
      const { code } = res.data;
      if (code === '200') {
      ...
      yield put({
        ...
        payload: {
          page: {
            ...
            orderby: (Object.keys(role.orderby).length === 0) ? {} :
              role.orderby,
          },
          key: role.searchInfo,
          storeids: role.storeid,
          status: role.status,
        },
      });
    },
    ...
    * judgeStatus({ payload }, { put, select }) {
      const { checkedRole } = yield select(state => state.role);
      const startArray = [];      // 启用按钮数组
      const blockArray = [];      //  停用按钮数组
      const judgeStartArray = checkedRole.filter(item => item.status === 1);
      const judgeBlockArray = checkedRole.filter(item => item.status === 0);
      ...
      yield put({
        type: 'updateState',
        payload: {
          deleteBtnStatus: !((checkedRole.length > 0)),
          startBtnStatus: (judgeStartArray.length === checkedRole.length),
          blockBtnStatus: (judgeBlockArray.length === checkedRole.length),
