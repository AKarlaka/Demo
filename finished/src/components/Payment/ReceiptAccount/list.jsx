  ...
  const columns = [{
    ...
    render: (text, record) => (
      <span>
        ...
        <Link to={`/business/receiptAccount/channel/${record.merchantId}/${record.id}`}>配置</Link>
        {
          record.state === State.ENABLE &&
          <span>
            <Divider type="vertical" />
            <Link to={`/business/receiptAccount/shop/${record.merchantId}/${record.id}`}>绑定门店管理</Link>
          </span>
        }
      </span>
    ),
